...
from src.entity.contacts.models import Contact
...
async def get_contact_by_id(contact_id: int, db: AsyncSession) -> Contact | None:
...
async def get_contact_by_value(
    ...
    if first_name:
        stmt = select(Contact).where(Contact.first_name.ilike(f"%{(first_name.lower())}%"))
    elif last_name:
        stmt = select(Contact).where(Contact.last_name.ilike(f"%{last_name.lower()}%"))
    elif email:
        stmt = select(Contact).where(Contact.email.ilike(f"%{email.lower()}%"))
...
async def update_contact(
    contact_id: int, body: ContactUpdateSchema, db: AsyncSession
) -> Contact | None:
...
async def full_update_contact(
    contact_id: int, body: ContactPutSchema, db: AsyncSession
) -> Contact | None:
